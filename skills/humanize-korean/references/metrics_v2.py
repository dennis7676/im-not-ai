...
#    0.355). 신호가 틀린 게 아니라 코퍼스가 안 맞는다 — 우리 "사람 코퍼스"가 산문이
#    아니라 프롬프트다. baseline 셀도 임계값도 만들지 않는다 — **관측용으로만 쓴다.**
...
def nominal_dominance(text: str) -> float:
    ...
    ⚠️ **쟀고, 이 코퍼스로는 검증이 안 된다** (2026-08-25). 홀드아웃 AUC 0.275 —
    0.5 미만이므로 방향이 반대다(사람 중앙값 0.901 대 AI 0.844). 원인은 신호가
    아니라 코퍼스다: 우리 "사람 코퍼스"는 산문이 아니라 프롬프트라 명사를 나열하고
    조사를 생략한다(사람 nominal 0.761 대 AI 0.677). 화행이 교란변수다.
    **뒤집어 쓰면 된다고 읽지 말 것** — 그러면 저자가 아니라 지시문을 판별한다.
...
def spacing_uniformity(text: str) -> float:
    ...
    ⚠️ **쟀고, 이 코퍼스로는 검증이 안 된다** (2026-08-25). 홀드아웃 AUC 0.355 로
    방향이 반대였다 — 사람 쪽이 더 균일했다(0.459 대 AI 0.490). `nominal_dominance`
    와 같은 원인이고 같은 처분이다: 프롬프트는 어절이 고르다. 임계값도 baseline 셀도
    만들지 않는다 — **관측만 한다.**
    전문: `docs/validation_2026-08-25_word-precision-signals.md`
    """
    body = _strip_markup(text)
