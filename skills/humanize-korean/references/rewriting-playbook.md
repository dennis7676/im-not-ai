...
7. **빼기 우선, 복원은 근거 안에서(Removal-First, Grounded-Restoration)**: 치환은 기본적으로 AI 티를 빼는 방향이다. 처방 예시의 어휘를 원문에 없던 자리에 새로 심지 않는다. 원문보다 격식을 올리지 않는다 — '-했-' → '-하였-' 금지, '~인데요/~거든요/~한 겁니다' 구어 종결 보존. 살아있는 구어는 사람 글의 증거다.
8. **명료성 복원(Clarity Restoration · M절)**: 다만 원문이 **과압축**이면(조사·어미 탈락, 서술어 부재, 성분 생략) 빼기만으로는 못 고친다. 이때는 넣는다 — 단, 넣을 근거가 원문의 다른 문장이나 앞뒤 문단에 있을 때만. 근거 없는 자리는 그대로 둔다. **판별 한 문장: 넣은 말이 새 주장을 만들면 위반, 있던 주장을 읽히게 만들면 복원이다.**
...
### M. 명료성 복원 레시피 (과압축 교정 · fork 로컬 확장)

> 앞의 A~J와 방향이 반대다. 여기서는 **넣는다.** 근거는 원문 안에만 있고, 산문 문장에만 적용한다 — 헤딩·표 셀·개조식 항목·코드 텍스트는 손대지 않는다.

| 원문 패턴 | 윤문 예시 | 본진 ID |
|---|---|---|
| 그러면 경고가 붙습니다 | 그러면 이미 작업 중인 파일에도 경고 표지가 추가됩니다 | M-1 |
| 압축 전 반영한다 | 컨텍스트가 압축되기 전에 이 결정을 반영해 두겠습니다 | M-1·M-3 |
| 테스트 통과. 배포 대기 중. | 테스트는 모두 통과했고, 지금은 배포를 기다리는 중입니다 | M-2 |
| 캐시 재과금 방지 위해 세션 경계에서 전환 | 캐시가 재과금되지 않도록 세션 경계에서 모델을 전환합니다 | M-2·M-3 |
| 사본의 문구는 작업의 상황을 | 사본에 기재된 문구는 작업이 진행되는 상황을 | M-4 |
| 추론 용도의 토큰 카운트 함수의 오류 상황에서 | 지출한 비용을 추론하는 토큰 카운트 함수에 오류가 발생하면 | M-4 |
| 분석의 흐름 | 분석의 방향 | M-5 |
| 코드로 박는 자리 | 코드에 명시하는 위치 | M-5 |
| 요청을 받습니다 | 요청을 확인했습니다 / 요청대로 진행하겠습니다 | M-5 |
| 캐시를 갉는다 / 컨텍스트를 삼킨다 | 캐시를 잠식한다 / 컨텍스트를 차지한다 | M-5 |
| 기제 / 정합하다 / 가늠자 | 장치 / 들어맞다 / 기준 | M-6 |

**복원 우선순위** — 한 문장에 여러 결손이 겹치면 이 순서로 고친다. 서술어(M-2) → 격조사(M-3) → 빠진 성분(M-1) → 관형격 풀기(M-4) → 어휘 교체(M-5·M-6). 어휘부터 손대면 문장 구조가 잡히기 전에 의미가 흔들린다.

**멈추는 자리** — 빠진 주어가 "나인지 시스템인지 사용자인지" 원문으로 판정되지 않으면 복원하지 않는다. 이 경우 문장을 그대로 두고 finalize 콜에 넘긴다. 잘못 세운 주어 하나가 보고문 전체의 책임 소재를 바꾼다.
