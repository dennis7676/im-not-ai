...
> **로컬 fork 표기 (2026-08-20, 2026-08-25 갱신):** 본 파일은 `dennis7676/im-not-ai` fork이며 본진 A~J는 upstream 원문 그대로다. 로컬 확장은 **K절·L절·M절 셋뿐**이고 본진을 덮어쓰지 않는다는 것이 병합 규칙이다. K절은 DP 개인글(블로그·에세이·연재)에만 적용, L절은 장르 불문 로컬 코퍼스 실측 기반 일반 확장, M절은 과압축 교정(fluent-korean 이식)이다 — 적용 범위와 방향이 다르므로 절을 분리했다. **M절만 유일하게 본진과 방향이 역전되는 지점이 있어 자체 충돌 조정표를 가진다.** upstream을 당길 때 충돌은 K·L·M절과 목차 세 줄에서만 난다. 2026-08-31 갱신.
...
J. 시각 장식 남용 — J-1~J-4
K. 로컬 확장 — DP 개인글 전용 (fork) — K-1~K-3 + 오버라이드
L. 로컬 확장 — 포괄어·구체성 (fork, 장르 불문) — L-1
M. 로컬 확장 — 명료성 복원 (과압축 교정, fork, 장르 불문) — M-1~M-7 + 충돌 조정표
...
> **적용 범위 경계.** 이 절은 이 저장소의 fork에서만 존재하는 로컬 확장이며, **대웅님 명의 개인글(블로그·연재·에세이·영상 자막)에만** 적용한다. 업무 문서·리포트·명세, 그리고 다른 사용자의 글에는 적용하지 않는다 — 거기서는 명사형 압축이 오히려 읽기 쉽고, 소비자가 다르면 목소리도 달라야 한다.
>
> **허구 서사(소설·웹소설·시나리오 대사)에는 적용하지 않는다** (2026-08-21 사용자 승인). 대사는 구어여야 하고 헤지 규칙 자체가 인물 목소리와 충돌한다. 훅 `ai-tell-guard.py`의 `FICTION_PATTERNS`가 같은 제외를 결정론으로 집행한다 — 지금은 대상 경로 밖이라 우연히 안 걸리는 상태였고, 경로를 넓힐 때 딸려 들어가지 않도록 제외를 규칙으로 남겼다.
>
> 근거 SSOT는 볼트 `300_Areas/310_Personal/Digital_Me/writing-persona.md` 6-1절(금칙)과 2절(레지스터 정본 E1~E6)이다. 여기 K-1~K-3은 그 6-1절 금칙 중 **본진 A~J가 잡지 못하는 것만** 옮긴 것이고, 겹치는 금칙은 옮기지 않았다(아래 매핑 표).
>
> 결정론 1차 방어선은 별도로 있다 — `~/.claude/hooks/ai-tell-guard.py`(PostToolUse `Write|Edit`)가 발행 원고 경로에서 6패턴을 자동 검사한다. **훅은 감지, 이 스킬은 수리**로 역할이 갈린다.

#### K-0. 본진 매핑 (중복 등재하지 않은 금칙)

| writing-persona 6-1절 금칙 | 본진 커버 | 비고 |
|---|---|---|
| 1. 명사 나열로 끝내는 제목 | A-13 | 그대로 사용 |
| 2. 한자어 대구 | 부분(C-8·F-4) | 2음절 한자어 병렬은 미커버 → **K-3 신설** |
| 3. 문장 길이가 다 같음 | E-1·E-4 | 그대로 사용 |
| 4. "~할 수 있습니다 / ~가 필요합니다" 남발 | A-10·I-5 | 그대로 사용 |
| 5. 첫 문장부터 결론 | C-4 | 그대로 사용 (DP 3절 "설명형 진행"과 같은 규칙) |
| 6. 모든 항목을 같은 꼴로 맞춘 목록 | C-1·E-2 | 그대로 사용 |
| 7. 무생물이 행동하게 두지 않는다 | A-15·D-5 | 그대로 사용 |
| 8. 수사적 설정을 깔지 않는다 | 없음 | **K-1 신설** |
| 9. 메타 조인트를 쓰지 않는다 | 부분(D-1 종결 요약만) | 담화 표지는 미커버 → **K-2 신설** |

### K-1. 수사적 설정 — 질문 던지고 자답 [S1] · fork 신규

- 패턴: 독자에게 질문을 던진 뒤 필자가 곧바로 답하는 구조. 강연 대본의 습관이지 글의 습관이 아니다.
- 예: "공통점이 **보이시나요**?" / "**왜 중요하냐면**요," / "그럼 어떻게 해야 **할까요**?" / "여기서 질문 하나. ~"
- 처방: 질문을 지우고 바로 본론으로 간다. 대개 답 문장만 남겨도 뜻이 온전하고 글이 빨라진다. 진짜 미해결 질문이라 남길 때는 **자답하지 않고** 그대로 둔다.
- 주의: DP 2절 E3(대비축 사고)의 "두 개념을 세우고 그 사이를 찾는" 전개와 혼동하지 않는다. E3는 질문이 아니라 대조로 연다.
- **반례 — 답을 다음 편으로 넘기는 피벗은 K-1이 아니다** (2026-08-21 사용자 판정, 온톨로지 1편):

  > 그럼 어떻게 적어두면 되느냐. 방법은 이미 나와 있어요. 아주 오래됐어요. 여러분도 하나쯤 써보셨을 거예요. 명절마다 봤던 그 그림이요.
  >
  > 다음 편에서 꺼내볼게요.

  형태는 질문이고 바로 뒤에 문장이 따라오지만 **자답이 아니다.** "방법은 이미 나와 있어요"는 답이 아니라 답이 있다는 예고이고, 실제 방법은 다음 편으로 넘어간다. 사용자 판정 원문: **"K-1의 해악은 자기가 바로 답해서 가르치는 톤인데, 여기는 답을 안 준다."**
- **따라서 판별 기준은 질문의 형태가 아니라 "같은 글 안에서 자기가 답하는가"다.** 답이 그 글 안에 있으면 K-1, 다음 편·다음 절로 넘어가면 정상적인 피벗이다. 윤문 콜이 이 둘을 구분하지 못하면 삭제하지 말고 그대로 둔다 — 클리프행어를 지우는 손해가 수사적 설정을 남기는 손해보다 크다.
- 실측 경위: 이 사례를 처음 걸었을 때 스킬은 뒤따르는 단문 넷이 클리프행어를 대신 수행한다고 보고 삭제했다. 사람이 그 판단을 뒤집었다.
- _quick: true · quick_pattern: 질문형 종결 직후 같은 화자가 답하는 문장(보이시나요/할까요/왜 중요하냐면) · quick_fix: 질문 문장 삭제하고 답 문장부터 시작_

### K-2. 담화 메타 조인트 [S2] · fork 신규

- 패턴: 글의 구조를 독자에게 안내하는 표지. 독자는 지금 읽고 있는 중이라 안내가 필요 없다.
- 예: "**정리할게요**." / "**앞서 말했듯**" / "**이 글에서는** ~를 다룹니다" / "**다시 말해**" / "**본격적으로 들어가기 전에**"
- 처방: 지운다. 지우면 대개 문장이 바로 이어지고 글이 빨라진다. 앞 내용을 정말 다시 불러야 하면 지시어가 아니라 **그 내용의 구체어**로 부른다("앞서 말한 그 문제" → "지도 없이 쓴 그 노트").
- 본진 D-1과의 경계: D-1은 "결론적으로·요약하면" 같은 **종결 요약 lexicon**을 잡는다. K-2는 글 중간의 **담화 표지**를 잡는다. 둘 다 걸리면 가산한다.
- _quick: true · quick_pattern: 담화 표지(정리할게요/앞서 말했듯/이 글에서는/본격적으로 들어가기 전에) · quick_fix: 삭제하고 문장을 바로 잇기, 재호출은 구체어로_

### K-3. 한자어 2음절 대구 [S2] · fork 신규

- 패턴: 2음절 한자어 둘을 "과/와"로 묶어 개념을 압축하는 습관. 보고서 문체의 기본 동작이고, 개인글에서는 한 문장에 하나만 있어도 톤이 튄다.
- 예: "**구축과 운영**" / "**품질과 효율**" / "**범위와 방법**" / "**확장과 유지**"
- 처방: **둘 중 하나는 풀어 쓴다.** "구축과 운영이 어렵다" → "만드는 것보다 늙지 않게 두는 게 어려운 것 같아요"(2026-08-19 실제 교정 짝). 대구를 살려야 할 때는 리듬을 위해 길이를 일부러 어긋나게 둔다.
- 본진 C-8("A인가, B인가")·F-4(한자 접미사)와 겹치지 않는다 — 이쪽은 **병렬 명사구** 자체가 대상이다.
- _quick: false_

#### K-4. 오버라이드 — 본진 규칙 중 DP 개인글에서 다르게 적용되는 것

> **이 표가 이 fork의 존재 이유다.** 본진을 그대로 적용하면 대웅님 목소리의 핵심(헤지로 열고 여운으로 닫는 것)이 AI 티로 오인돼 깎인다. 아래는 완화이지 면제가 아니다 — 상한은 그대로 지킨다.

| 본진 | 기본 판정 | DP 개인글 오버라이드 | 근거 |
|---|---|---|---|
| **G-1 추측·관측형 종결** | 티 | **열기·본문에서는 유지한다.** "~인 것 같아요", "~라는 생각이 들어요"는 DP 레지스터의 정본(E1·E5)이지 AI 티가 아니다 | writing-persona 2절 E1·E5 |
| **G-2 이중·삼중 완곡** | 티 | 그대로 적용. 한 문장에 헤지 둘 이상은 DP 기준으로도 물러선 문장이다 | — |
| **G-1 적용 예외의 예외 — 착지** | — | **글의 최종 판단 한 줄은 확신으로 착지한다.** 마지막 문장이 헤지로 끝나면 우유부단으로 읽힌다. 본문 헤지=진솔, 착지 헤지=자신감 결여 | E2-예외 (2026-07-07 교차검증) |
| **"~인 셈이에요" / "~라고 할 수 있어요"** | D-1 | **편당 1회 상한.** 세 번 나오면 문장이 전부 한 발 물러나 있는 느낌이 된다 | 6-1절 |
| **E-2 동일 종결어미 반복** | 티 | 해요체는 고정이다(레지스터). 종결어미 자체가 아니라 **어휘와 문장 길이**로 다양성을 만든다 | 4절 |
| **E-7 청자 경어법 일관성** | 티 | 정합. 해요체 단일 유지, 합쇼체("습니다") 혼입이 오히려 위반 | 4절 각주 |
| **E-1 문장 길이 표준편차** | 티 | 강화 적용. 긴 문장 뒤에는 **반드시** 짧은 문장 하나(스타카토) | 3절 |
| **J-3 대시(—) 남용** | 티 | 그대로 적용하되 전면 금지는 아니다. 영어 스킬의 대시 전면 금지는 옮기지 않았다 | 6-1절 출처 각주 |
| **F-1 정도부사 중독** | 티 | 남용 기준만 적용. **부사 전면 금지는 옮기지 않는다** — 한국어에서 부사는 리듬을 만든다 | 6-1절 출처 각주 |
| **C-5 이모지 / D-4 hype 어휘** | 티 | 강화 적용. DP 5절 금칙이 이미 원칙 미사용 | 5절 E6 |

---

## L. 로컬 확장 — 포괄어·구체성 (fork, 장르 불문, 2026-08-25)

> **적용 범위 경계.** K절과 달리 이 절은 DP 개인글에 한정되지 않는다 — 에세이·리포트·업무 문서 등 **산문 단락을 포함하는 모든 장르**에 적용한다. 근거 SSOT는 `RESEARCH/korean-word-precision_20260825_065209/artifacts/agent_results/`(A0~A4)이고, 계측기는 `references/metrics_v2.py` v2.4 로컬 확장이다.

### L-1. 포괄어 남용 (Vague/Generic Noun) [S2 · 사용자 취향] · fork 신규

- 패턴: 상태·대상·축·자리·지점·층처럼 여러 의미를 두루 덮는 명사를 써서, 그것이 정확히 무엇을 가리키는지가 문장에도 문맥에도 없는 상태. 개별 출현은 무해하지만, **한 문장에 둘 이상 몰리면서 지목 가능한 구체 신호가 하나도 없을 때** 문장이 통째로 흐려진다.
- 예 (2026-08-25 사용자 지적, 한 문장에 셋이 몰린 사례): "고친 여섯 건이 전부 같은 **모양**이었어요. **기제**는 만들어져 있고 절반만 연결된 **자리**들이라, 겉으로는 돌아가지만 답이 필요할 때 아무것도 안 알려주던 것들입니다." — 무엇이 같은 모양인지, 무엇이 만들어졌는지, 어디가 자리인지가 문장에도 앞뒤 문맥에도 없다.
- **근거 등급 (정직하게 표기)**: 한국어 문체론에서 이 포괄어 목록 자체를 금지어로 지목한 1차 출처는 **찾지 못했다**(A1 4절 — "것/부분/측면/방식/상황/경우/문제/내용 같은 범용 명사의 남용 여부: 이번 조사에서 이 단어들 자체를 금지·경계 대상으로 지목한 1차 출처를 찾지 못했다"). 국립국어원 공공언어 지침이 확립한 것은 번역투 조사·연결형(~에 대해/~을 통해/~에 있어서)·서술어 없는 명사 나열·명사형 종결(~함/~음)이지, 포괄어 목록이 아니다. **따라서 L-1은 학술 근거 규칙이 아니라 사용자 취향 규칙이다** — A~K의 다른 항목과 같은 급의 "확립된 규범"으로 인용하지 않는다.
  - 인접 근거 (방향은 지지, 목록은 지지하지 않음): Hayakawa 추상 사다리(A3 1.3절) — 한국어 판정 기준으로 옮기면 "지목 가능한가"(육안·측정·이름을 댈 수 있는 대상인가). plainlanguage.gov·EU "How to write clearly"·Microsoft/Google 스타일 가이드의 vague noun 규정(thing/area/issue/aspect/factor류, A3 1.4·2.1절) — "단어가 유발하는 오해의 폭을 줄인다", "오독 가능한 대명사·지시어는 구체 명사로 치환한다"는 **원칙만** 차용하고 영어 단어 목록은 이식하지 않는다.
- **로컬 실측** (사용자 발화 276,770자 대 에이전트 응답 2,814,379자 대조, 10만자당 빈도, A0):
  ...
  목록(`metrics_v2.py`의 `_VAGUE_NOUNS`)은 10만자당 차이 +3 이상만 채택(상태·대상·축·자리·지점·층 + 측면·차원·셈). "모양"·"기제"는 총빈도로는 걸리지 않는다 — 문제는 **문장 내 밀도**였다.
- **반증 2건 (목록을 넓히지 않는 이유)**:
  1. 사용자가 처음 지목한 "모양"·"기제"는 총빈도로는 문제가 아니다. 오히려 에이전트가 덜 쓴다 — 밀도 문제와 빈도 문제를 혼동하지 않는다.
  2. "구조(47회)·부분(26)·방식(21)·상황(21)"은 사용자 발화에도 상위 빈도로 나온다. **금지 목록에 넣으면 사람 글이 걸린다** — `_VAGUE_NOUNS`에서 의도적으로 제외.
- **기각·조건부 판정된 가설**:
  - 문단당 포괄어 밀도로 게이트 → **기각**(사용자·에이전트 중앙값 0.00 동일, 판별력 없음).
  - "포괄어 2개+ 문장 내 구체 신호 0" 규칙 → **조건부 채택**. 케이스는 갈랐으나 전수 코퍼스에 그대로 걸면 적중률이 사용자 0.38% 대 에이전트 0.09%로 **역전**된다. 원인은 기술 문장에 코드·경로·수치가 항상 끼어 있어 "구체 신호 있음"으로 자동 통과하는 것 — **산문 단락에만 유효**하고 기술 문장·코드 스니펫·개조식 항목에 걸면 사람 글이 더 많이 걸린다.
- **장르 힌트**: 에세이·리포트 본문·서사 산문에 적용. 코드·경로·수치·표가 섞인 기술 문장, 개조식 보고서 항목, 법률·계약 조문에는 적용하지 않는다 — 구체 신호가 항상 있어 판별력이 뒤집힌다.
- 처방(금지가 아니라 치환): 그 자리에 **지목 가능한 이름을 넣을 수 있는가**를 먼저 묻고, 넣을 수 있으면 넣는다.
  - 자리 → 코드 위치, 호출 지점, 슬롯
  - 방식 → 절차, 접근법, 알고리즘
  - 모양 → 유형, 패턴
  - 기제 → 훅, 게이트, 장치
  - 감각적 대체어로 "**덩어리**"(사용자 22회, 에이전트 거의 미사용)를 우선 고려한다 — 포괄어 자리를 구체적 질감으로 대신할 수 있다.
- 계측: `metrics_v2.py`의 `vague_noun_rate`(10만자당 빈도, 실측 3.3배 차이·진단 신호 전용) · `vague_noun_unsupported`(포괄어 2개+ 구체 신호 0 문장 수, **산문 전용**, 게이트 아님). 같은 조사에서 `noun_string_max`도 계측됐으나 **별도 패턴으로 승격하지 않는다**(근거는 plainlanguage.gov 명사 나열, A3 4절). `light_verb_rate`는 **2026-08-25 제거됐다** — 전수 코퍼스 0.0, 홀드아웃 AUC 0.500, 클로드·코덱스 8편 전부 0.0 으로 세 방법이 모두 무신호였다.
- 승격 근거 메모: 통상 승격 기준("실증 사례 2건 이상")은 스팬 개수가 아니라 **로컬 코퍼스 정량 실측**(A0)으로 충족했다고 판단했다 — 사용자가 지적한 예문 1건 + 10만자당 빈도 대조 + 반증 2건 + 조건부 규칙 1건, 총 4갈래 증거가 스팬 2건보다 많은 정보를 준다.
- fast 경로 미포함 사유: 산문 전용 패턴을 전수 적용하면 역전이 실측됐다(위 "기각·조건부 판정된 가설" 참조) — 표층 게이트로 내리지 않고 strict 진단(humanize-diagnostician)이 문서·문단 맥락을 보고 판단한다.
- _quick: true · quick_pattern: 포괄 명사(자리, 지점, 층, 상태, 대상, 축, 기제, 모양, 측면, 차원, 셈)가 지목 대상 없이 쓰임 — 단독 출현도 대상 · quick_fix: 지목 가능한 실제 이름으로 치환, 못 넣으면 문장을 서술로 풀어 쓴다. 정본 references/banned-lexicon.md_

## M. 로컬 확장 — 명료성 복원 (과압축 교정, fork, 장르 불문, 2026-08-31)

> **왜 새 절인가.** A~L은 전부 **과잉**을 덜어내는 규칙이다. 수사가 많고, 완곡이 많고, 접속사가 많고, 장식이 많다. 그런데 코딩 에이전트가 뱉는 한국어의 지배적 실패는 반대쪽에 있다 — **과압축**이다. 토큰을 아끼도록 조율된 모델은 조사와 어미를 떨어뜨리고, 명사를 붙여 쌓고, 서술어 없이 문장을 끝내고, 일반 동사 자리에 비유 동사를 꽂는다. 결과물은 AI 티가 아니라 **판독 비용**으로 나타난다. 덜어내기만 하는 룰북으로 이 글을 윤문하면 더 짧아지고 더 안 읽힌다.
>
> **계보.** [snflkd/fluent-korean](https://github.com/snflkd/fluent-korean) output-style(MIT, 국어국문학 전공자 작성)의 조항을 taxonomy 형식으로 이식하고, 본진과 충돌하는 지점을 아래 조정표로 갈랐다. 원 저장소는 사전 규율(system prompt)이고 이 절은 사후 윤문(post-hoc)이다 — 같은 목표를 다른 층에서 집행한다.
>
> **적용 범위.** L절과 같이 장르를 가리지 않는다. 다만 **산문 문장에만** 적용하며, 헤딩, 표 셀, 개조식 목록 항목, 코드 주석, 커밋 메시지, 변수명, 로그 문자열은 전부 제외한다. 이 제외는 M-2에서 특히 중요하다 — 개조식 항목에 서술어를 붙이면 문서가 망가진다.

### 본진 충돌 조정표 (M절 신설로 생긴 방향 역전)

| 본진 항목 | 본진이 시키는 것 | M절이 시키는 것 | 조정 |
|---|---|---|---|
| E-2 종결어미 반복 | "명사형 종결을 섞어" 변주 | M-2가 명사형 종결 금지 | **E-2의 명사형 종결은 리듬 변주 1~2회 한정.** 보고, 설명, 판정 문장의 기본 종결로는 쓰지 않는다. 변주가 필요하면 시제, 상, 양태 어미를 먼저 쓴다 |
| F-4 과잉 접두, 접미 / F-5 "~적 N" 체인 | 한자어 명사화 접미사 억제 | M-4가 구체 한자어 활용 권장 | **층이 다르다.** F는 "-성, -적, -화"를 덧붙여 만든 **파생 추상어 연쇄**를 막고, M-4는 이미 존재하는 **구체 한자어 술어 선택**을 권한다. "가시성 확보의 필요성"은 F 대상, "누락된 호출을 탐지한다"는 M-4가 지향하는 결과다 |
| L-1 포괄어 남용 | 흐린 명사를 구체어로 | M-6이 튀는 어휘를 통용어로 | **짝패턴.** L-1은 하한(너무 흐림), M-6은 상한(너무 낮은 빈도). 둘 사이 구간이 목표다. 치환 후보가 L-1과 M-6 양쪽에 걸리면 통용 빈도가 높은 쪽을 택한다 |
| K절 DP 개인글 오버라이드 | 명사형 압축 허용(업무 문서) | M-2 비완결 종결 금지 | **K절이 이긴다.** DP 명의 개인글, 연재, 자막에서는 M-2를 적용하지 않는다. 목소리가 먼저다 |
| 주의사항 "register 보존 — 양방향" | 구어 입력이면 구어 출력 | 원저장소는 "사용자 어조 모방 금지" | **본진이 이긴다.** 원저장소는 어시스턴트 발화를 규율하는 output-style이라 어조 모방 금지가 성립하지만, 이 스킬은 남의 원고를 윤문한다. 필자의 구어는 보존 대상이지 교정 대상이 아니다 |

### M-1. 문장 성분 생략 — 정보 결손 압축 [S2] · fork 신규

- 패턴: 주어, 목적어, 부사어 중 의미를 지탱하는 성분이 빠져서 읽는 쪽이 맥락을 추론해야 하는 문장. 무엇이 어디에 어떻게 되는지 중 둘 이상이 문장에도 앞뒤 문맥에도 없다. AI 티 특유의 화려함이 아니라 **판독 비용**으로 나타나기 때문에 A~L 어느 항목에도 걸리지 않고 통과한다.
- 예: "그러면 경고가 붙습니다." → 무엇에 어떤 경고가 붙는지 없음 → "그러면 이미 작업 중인 파일에도 경고 표지가 추가됩니다."
- 예: "압축 전 반영한다." → "컨텍스트가 압축되기 전에 이 결정을 반영해 두겠습니다."
- 처방: 빠진 성분을 **원문의 맥락 안에서만** 복원한다. 앞뒤 문단에 근거가 없으면 복원하지 말고 그대로 둔다 — 없는 정보를 지어내는 것이 생략보다 나쁘다. 판단이 서지 않으면 finalize 콜에 넘긴다.
- 근거: 국립국어원 공공언어 지침이 확립한 "서술어 없는 명사 나열" 계열의 확장. L-1 근거 메모에서 이미 **확립된 규범**으로 인용된 축이다.
- 주의: 이 처방은 글자 수를 **늘린다.** 변경률 게이트가 부풀려지므로 M절이 지배 패턴일 때는 경고 해석을 조정한다(SKILL.md 게이트 절 참조).
- **복원을 포기해도 문장을 통째로 면제하지 않는다.** 보류 대상은 그 문장의 **빠진 성분 하나**이지 문장 전체가 아니다 — E-2와 A~L의 다른 처방은 그대로 적용한다.
  - ⚠️ **이 조항의 최초 근거는 틀렸다** (2026-08-31 재eval로 자기 정정). 처음에는 픽스처의 격식 혼재가 남은 것을 보고 "M-1의 보류가 E-7의 면제로 번졌다"고 진단했는데, 재실행한 작업자가 이유를 명시했다 — **E-7 자체가 장르 가드로 보고문을 제외하고 있어서** 애초에 적용 대상이 아니었다. 조항 자체는 옳아서 남기되, 그 사례는 M-7이 담당한다. 진단을 고칠 때 조항까지 함께 지우지 않는다.
- _quick: true · quick_pattern: 주어 또는 목적어 부재로 지시 대상이 문맥에도 없는 서술 문장 · quick_fix: 원문 맥락 안에서만 성분 복원, 근거 없으면 그 성분만 보류(문장의 격식과 다른 처방은 그대로 적용)_

### M-2. 비완결 종결 — 명사구, 부사구, 연결어미 끝맺음 [S2] · fork 신규

- 패턴: 산문 문장이 서술어와 종결어미 없이 끝난다. 명사형 종결("~함", "~됨", "~설정 완료"), 부사구 종결("~하도록", "~보다 빠르게"), 연결어미 종결("~하고", "~인데", "~지만")이 모두 해당한다. 개조식 문서에서 넘어온 습관이 산문 단락까지 침범한 상태다.
- 예: "캐시 재과금 방지 위해 세션 경계에서 전환." → "캐시가 재과금되지 않도록 세션 경계에서 모델을 전환합니다."
- 예: "테스트 통과. 배포 대기 중." → "테스트는 모두 통과했고, 지금은 배포를 기다리는 중입니다."
- 처방: 서술어를 세우고 종결어미로 닫는다. 명사형이 꼭 필요하면 그 문단 안에서 1회로 제한한다.
- **적용 제외**: 헤딩, 표 셀, 개조식 목록 항목, 캡션, 코드 주석, 커밋 메시지, 로그 문자열. 이 자리에서는 명사형 압축이 정답이다.
- 근거: 국립국어원 공공언어 지침의 명사형 종결(~함, ~음) 경계 규정. fluent-korean 문장 단위 2항.
- _quick: true · quick_pattern: 산문 문장이 명사형, 부사구, 연결어미로 종결(헤딩과 개조식 항목 제외) · quick_fix: 서술어 세우고 종결어미로 닫기, 명사형은 문단당 1회_

### M-3. 조사, 어미 탈락 — 전보문체 [S2] · fork 신규

- 패턴: 문장 성분은 남아 있는데 그것들을 잇는 조사와 어미가 빠져 관계가 불분명한 상태. A-13(명사 나열)의 짝패턴이다 — A-13은 명사를 **붙여 쌓는** 것이고, M-3은 붙이지는 않았으나 **조사를 떨어뜨린** 것이다. 보조 용언과 선어말어미까지 함께 증발해 시제, 상, 태가 사라지는 경우가 많다.
- 예: "이 결정 이후 중요 정책 갈리는 자리. 컨텍스트 압축 전 신중 반영한다." → "이 결정은 이후 중요한 정책에도 지속적으로 영향을 주기 때문에, 컨텍스트가 압축되기 전에 신중하게 반영하겠습니다."
- 처방: 격조사와 보조사를 복원하고, 부사와 보조 용언, 선어말어미로 관계와 시제를 명시한다. "-겠-", "-었-", "-어 두다", "-어 놓다", "-고 있다"를 아끼지 않는다. 다만 "-고 있다"는 E-2 v2.0 보강과 충돌하므로 **진행 의미가 실제로 필요할 때만** 쓴다.
- 근거: fluent-korean 구 단위 1항. 국립국어원 공공언어 지침의 조사 생략 경계.
- _quick: true · quick_pattern: 격조사 탈락으로 성분 관계가 불분명한 서술 문장, 시제와 태 표지 부재 · quick_fix: 격조사와 보조사 복원, 보조 용언과 선어말어미로 관계 명시_

### M-4. 관형격 '의' 연쇄 — 의미 관계 은폐 [S2] · fork 신규

- 패턴: 서술 관계로 풀어야 할 자리를 "~의"로 접어서 두 명사 사이의 관계가 무엇인지(주격인지 목적격인지 소유인지) 사라진 상태. "~의"가 한 문장에 두 번 이상 나오면 대개 성분 하나가 함께 증발해 있다 — M-1의 잦은 발생 경로다. A-19(이중 조사 "~에서의")와 구별한다: A-19는 조사 결합, M-4는 순수 관형격 연쇄다.
- 예: "사본의 문구는 작업의 상황을 반영한다." → "사본에 기재된 문구는 작업이 진행되는 상황을 반영합니다."
- 예: "지출 비용 추론 용도의 토큰 카운트 함수의 오류 상황에서" → "지출한 비용을 추론하는 토큰 카운트 함수에 오류가 발생하면"
- 처방: "~의"를 관형절("~하는", "~에 기재된")이나 서술절로 푼다. 문장당 "~의"는 2회 이하를 목표로 한다. 푸는 과정에서 **구체 한자어 술어**를 골라 의미를 명확히 한다("추론 용도의" → "추론하는", "오류 상황에서" → "오류가 발생하면").
- 이 항목이 F-4, F-5와 갈리는 지점: 여기서 권하는 한자어는 **동사로 활용되는 구체어**(반영하다, 발생하다, 탐지하다)이고, F가 막는 것은 **명사화 접미사로 부풀린 추상어**(가시성, 필요성, 고도화)다.
- _quick: true · quick_pattern: 한 문장에 관형격 "~의" 2회 이상 또는 명사+의+명사+의 연쇄 · quick_fix: 관형절과 서술절로 풀고 구체 한자어 술어 선택, 문장당 2회 이하_

### M-5. 비유적 어휘의 일반어 대체 [S1] · fork 신규

- 패턴: 일반적인 명사나 동사를 써야 할 자리에 비유 어휘를 꽂아 의미가 변질되거나 여러 갈래로 읽히는 상태. Opus, Fable 계열에서 특히 강하게 나타나며, 기술 문서에서 **오독의 직접 원인**이 된다. 글에 생기를 주려는 의도가 오히려 판독 비용을 올린다.
- 예: "분석의 흐름" → "분석의 방향" · "코드로 박는 자리" → "코드에 명시하는 위치" · "요청을 받습니다" → "요청을 확인했습니다" 또는 "요청대로 진행하겠습니다"
- 예(과한 현장감 구어): "박아넣다, 치우다, 얹다, 갉다, 삼키다" → "명시하다, 제거하다, 추가하다, 잠식하다, 흡수하다"
- 처방: 그 어휘가 **문자 그대로 읽혔을 때 문장이 성립하는가**를 묻는다. 성립하지 않으면 일반 어휘로 바꾼다.
- **예외(그대로 둔다)**: 일상적 문어에서 통용되고 해당 분야에서 관용으로 정착해 일반어로 바꾸면 오히려 어색해지는 표현(병목, 흐름 제어, 서버가 죽다, 로그를 남기다). 그리고 필자가 의도적으로 쓴 수사 — 원문에 이미 있던 비유는 K절, J-3의 구어 보존 예외와 같은 취급으로 **보존한다.** 이 항목은 **윤문기가 새로 얹는 비유**와 **AI가 일반어 자리에 꽂은 비유**를 대상으로 한다.
- 근거: fluent-korean 구 단위 3항 + 세부 동작 블록("사용 빈도가 낮아서 통용되지 않는 어휘").
- _quick: true · quick_pattern: 일반 명사나 동사 자리의 비유 어휘, 관용 정착 표현은 제외 · quick_fix: 문자 그대로 읽어 성립 여부 확인 후 일반 어휘로 치환, 원문에 있던 필자의 수사는 보존_

### M-6. 저빈도 현학 어휘 [S2] · fork 신규

- 패턴: 사전에 있고 뜻도 명확하지만 실제 사용 빈도가 낮아 통용되지 않는 어휘. 읽는 쪽이 한 번 멈춰야 하므로 소통 효율이 떨어진다. L-1(포괄어 남용)의 반대편 극단이다.
- 예: "기제" → "장치" 또는 "구조" · "정합하다" → "들어맞다" · "상재하다" → "함께 있다" · "가늠자" → "기준"
- 처방: 같은 뜻을 담으면서 통용 빈도가 높은 어휘로 바꾼다. 판정 기준은 **분야 종사자가 아닌 독자가 사전 없이 읽는가**이다.
- **예외**: 해당 분야의 표준 용어(멱등성, 역직렬화, 관형절), 법률, 학술 정의어, 고유명사. 이 자리에서 쉬운 말로 바꾸면 정확도가 깨진다. 전역 CLAUDE.md의 "쉬운 말(=학계 용어) 병기" 규칙이 우선한다 — 바꾸지 말고 병기한다.
- 주의: 이 항목은 **취향 규칙**이다. L-1과 같은 급이며 A~J의 확립된 규범과 같은 무게로 인용하지 않는다.
- _quick: true · quick_pattern: 통용 빈도가 낮은 어휘가 표준 용어 자리가 아닌 곳에 출현 · quick_fix: 통용 어휘로 치환, 분야 표준 용어와 학술 정의어는 병기로 보존_

### M-7. 문서 내 종결 격식 혼재 (산문, 장르 불문) [S2] · fork 신규 (2026-08-31 재eval)

- 패턴: 한 문서의 산문 문장이 합쇼체(~습니다), 한다체(~한다), 해요체(~해요) 사이를 오간다. 에이전트가 쓴 보고문, 인계 문서, 요약에서 빈출한다 — 원문 조각을 이어 붙이거나 문단마다 다른 관성으로 쓸 때 생긴다.
- 예 (2026-08-31 실측): "로그를 확인한 결과 3건이 실패했다. 원인은 타임아웃 120초 초과다. 재시도 로직은 없다. **그러면 경고가 붙습니다.**" — 한다체 세 문장 뒤 합쇼체 한 문장.
- **왜 E-7이 아니라 새 항목인가.** E-7(청자 경어법 일관성)은 본문 예시로 "같은 보고서가 한 문단은 '~합니다', 다음 문단은 '~한다'로 점프"를 정확히 들고 있는데, **검출 임계가 그것을 스스로 잘라낸다** — "장르 가드: 대화·구어 텍스트 한정 적용. 보고서·정책문 등 격식체 단일 장르는 본 패턴 미적용(이미 합쇼체로 일관)". 예시와 임계가 한 항목 안에서 모순이고, 실측은 괄호 안 전제("이미 일관")를 반증했다. 에이전트 보고문은 합쇼체로 일관하지 않는다.
  - 본진 E-7은 건드리지 않는다(upstream 병합 원칙). M-7이 로컬 확장으로 그 구멍만 덮는다. 대화·구어 텍스트는 그대로 E-7 소관이고, **M-7은 산문 문서의 종결 격식만 본다.**
- **실측 경로 (재현 가능하게 적는다)**: 1차 eval에서 claude, codex, agy 셋 중 둘이 이 혼재를 남겼다. 원인을 M-1의 보류로 오진해 M-1에 규칙을 넣고 재실행했더니 **결과가 그대로였고**, 작업자가 "E-7이 대화·구어 한정이라 원문 그대로 두었다"고 이유를 보고했다. 규칙을 고쳐도 결과가 안 바뀌면 진단이 틀린 것이다.
- 처방: 문서에서 다수를 차지하는 격식 하나로 통일한다. 소수 격식 문장을 다수 쪽으로 옮기며, 판정은 **문장 수 다수결**이다.
- **적용 제외**: 인용문(원문 화자의 격식 보존), 대화·인터뷰 트랜스크립트(E-7 소관), 헤딩·표 셀·개조식 항목, 그리고 필자가 의도적으로 격식을 바꾼 것이 문맥으로 드러나는 구간.
- 주의: register 보존 규칙과 충돌하지 않는다. register 보존은 **원문의 격식을 결과가 따르라**는 것이고, M-7은 **원문 안에서 갈라진 격식을 하나로 모으라**는 것이다. 다수 격식이 원문의 격식이다.
- 계측: `register_mix_rate`(소수 격식 문장 비율). 관측 전용이며 게이트가 아니다.
- _quick: true · quick_pattern: 산문 문장의 종결 격식(합쇼/한다/해요)이 한 문서에 2종 이상 혼재 · quick_fix: 문장 수 다수결로 하나에 통일, 인용문과 대화는 제외_
...
- **fork-local** (2026-08-31): **M절 신설 + M-1~M-6 신규 6건** — upstream 버전 번호는 바꾸지 않는다(K·L절과 같은 관례). [snflkd/fluent-korean](https://github.com/snflkd/fluent-korean) output-style(MIT) 전 조항을 taxonomy 형식으로 이식한 회차:
  - **신설 사유** — A~L은 전부 과잉을 덜어내는 규칙인데, 코딩 에이전트 한국어의 지배적 실패는 반대쪽 **과압축**이다. 조사·어미 탈락, 성분 생략, 비완결 종결, 비유 어휘 오용은 A~L 어느 항목에도 걸리지 않고 통과한다. 덜어내기 전용 룰북으로 윤문하면 글이 더 짧아지고 더 안 읽힌다.
  - **M-1** 문장 성분 생략(정보 결손 압축) · **M-2** 비완결 종결(명사구·부사구·연결어미 끝맺음) · **M-3** 조사·어미 탈락(전보문체, A-13의 짝패턴) · **M-4** 관형격 '의' 연쇄(A-19와 구별) · **M-5** 비유적 어휘의 일반어 대체 [S1] · **M-6** 저빈도 현학 어휘(L-1의 반대편 극단).
  - **본진 충돌 조정표 신설** — M절은 로컬 확장 중 유일하게 본진과 방향이 역전되는 지점을 가진다. 5건을 명문으로 갈랐다: E-2 명사형 종결 변주는 리듬용 1~2회로 한정 / F-4·F-5는 파생 추상어, M-4는 구체 한자어 술어로 층 분리 / L-1(하한)과 M-6(상한)은 짝패턴 / K절 DP 개인글은 M-2 미적용 / 'register 보존 양방향'이 원저장소의 '어조 모방 금지'를 이긴다(윤문 스킬은 남의 원고를 다루므로).
  - **quick: true 6건 전부** — 표층 신호로 탐지 가능하고 코딩 에이전트 산출물에서 빈도가 높다. quick 총계 51 → 57(예산 "50개 내외 ±20%" 상한 60 이내).
  - **변경률 게이트 상호작용** — M절 처방은 글자 수를 **늘린다**. M절이 지배 패턴인 회차는 변경률 경고(30%)가 과윤문이 아니라 복원 때문일 수 있으므로 SKILL.md 게이트 절에 해석 규칙을 명시했다.
  - **빌드 인프라** — `build_quick_rules.py`·`build_diagnosis_rules.py`의 카테고리·패턴 ID 정규식을 `[A-L]` → `[A-M]`으로 확장(L절 선례와 동일한 최소 변경).
  - **계측기 신설 및 route_hint 배선** (`metrics_v2.py` v2.7) — `nonfinal_sentence_rate`(M-2) · `genitive_dense_count`(M-4) · `compression_signal`(합성). **왜 필요했나**: `compute_route_hint()` 의 `tells` 는 어휘·피동 카운트라 **있는 것만** 센다. 2026-08-31 실측에서 전형적 과압축 보고문 3문단이 `risk_score=0 · route_hint=light` 로 나왔다 — 진단 콜을 생략하는 경로로 그대로 빠진다. 배선 후 같은 입력이 `standard` 로, 그 글을 M절 처방대로 복원한 판본은 `light` 를 유지했다(대칭 확인).
  - **계측기의 판별력 한계를 정직하게 기록한다** (표본 6편). 사람이 쓴 기술 지침 문서의 `nonfinal_rate` 가 0.20~0.58 로 넓게 퍼진다 — 기술 문서는 원래 명사 종결 메모가 많다. 인공 과압축 샘플은 0.75 였고 임계 0.60 은 **잠정값**이다. 그래서 **게이트로 승격하지 않고 baseline 셀도 만들지 않으며, route_hint 의 light 판정에만 건다** — 오탐 손해(진단 1콜)와 미탐 손해(최소 처리로 출고)가 비대칭이라서다. L-1의 `vague_noun_unsupported` 와 같은 취급이다.
  - **오탐 2건을 잡고 넘어갔다**: (1) `_strip_markup` 은 줄머리 마커를 **벗겨서** 내용을 살리므로 먼저 돌리면 개조식 줄이 산문으로 둔갑한다 — 원문 라인에서 먼저 걸러내도록 순서를 바꿨다. (2) 서지 인용 줄(영문 저자명·연도·출판사)이 41건 오탐 — 한글 음절 비율 50% 미만 문장을 제외했다.
- **fork-local** (2026-08-25): **L절 신설 + L-1 신규 1건** — upstream 버전 번호(v2.0.1)는 바꾸지 않는다(fork-only 변경이라 K절과 같은 관례). `RESEARCH/korean-word-precision_20260825_065209`(A0 로컬 코퍼스·A1 한국어 문체론·A2 장르 관습·A3 영어권 가이드·A4 AI 텍스트 실증) 5갈래 조사 결과 반영:
  - **L-1 포괄어 남용 (Vague/Generic Noun) [S2 · 사용자 취향] 신설** — 상태·대상·축·자리·지점·층 등 포괄 명사가 한 문장에 2개+ 몰리며 지목 가능한 구체 신호가 없을 때의 시그니처. **근거 등급을 정직하게 표기** — 한국어 문체론에 이 목록을 금지어로 지목한 1차 출처가 없어 학술 규칙이 아니라 사용자 취향 규칙으로 분류. 로컬 실측(사용자 27.7만자 대 에이전트 281.4만자)으로 목록 확정, 반증 2건(모양·기제는 총빈도 문제 아님 / 구조·부분·방식·상황은 목록에서 제외해야 사람 글이 안 걸림)과 조건부 가설(전수 코퍼스 적중률 역전 — 산문 전용) 함께 기록.
  - **quick: false** — 산문 전용 패턴을 전수 적용하면 사용자 0.38% 대 에이전트 0.09%로 **역전**되는 실측이 나와 fast 경로에 내리지 않는다. quick-rules.md는 이번 회차에 내용 변경 없음(빌드 재확인만).
  - **계측기**: `metrics_v2.py` v2.4 로컬 확장 — `vague_noun_rate`(진단 신호) · `vague_noun_unsupported`(산문 전용, 게이트 아님) · `noun_string_max`(관측만 — 별도 패턴 승격 안 함). `light_verb_rate`는 2026-08-25 제거(세 방법 모두 무신호).
  - **빌드 인프라 확장**: `scripts/build_quick_rules.py`·`build_diagnosis_rules.py`의 카테고리·패턴 ID 정규식을 `[A-K]` → `[A-L]`로 확장(L절 추가에 필요한 최소 변경, K절 신설 때와 동일한 선례).
