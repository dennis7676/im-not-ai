...
> **로컬 fork 표기 (2026-08-20, 2026-08-25 갱신):** 본 파일은 `dennis7676/im-not-ai` fork이며 본진 A~J는 upstream 원문 그대로다. 로컬 확장은 **K절·L절 둘뿐**이고 본진을 덮어쓰지 않는다는 것이 병합 규칙이다. K절은 DP 개인글(블로그·에세이·연재)에만 적용, L절은 장르 불문 로컬 코퍼스 실측 기반 일반 확장이다 — 적용 범위가 다르므로 절을 분리했다. upstream을 당길 때 충돌은 K·L절과 목차 두 줄에서만 난다.
