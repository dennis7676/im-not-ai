...
- 계측: `metrics_v2.py`의 `vague_noun_rate`(10만자당 빈도, 실측 3.3배 차이·진단 신호 전용) · `vague_noun_unsupported`(포괄어 2개+ 구체 신호 0 문장 수, **산문 전용**, 게이트 아님). 같은 조사에서 `noun_string_max`(이 코퍼스 판별력 없음 — 중앙값 4.0 동일, 관측만)와 `light_verb_rate`(이 코퍼스 0.0 — 죽은 지표, 관측만)도 계측됐으나 **별도 패턴으로 승격하지 않는다** — 근거(plainlanguage.gov 명사 나열·영어 nominalization 대응, A3 4절)는 유효해도 이 코퍼스에서는 대상이 없다.
...
- **fork-local** (2026-08-25): **L절 신설 + L-1 신규 1건** — upstream 버전 번호(v2.0.1)는 바꾸지 않는다(fork-only 변경이라 K절과 같은 관례). `RESEARCH/korean-word-precision_20260825_065209`(A0 로컬 코퍼스·A1 한국어 문체론·A2 장르 관습·A3 영어권 가이드·A4 AI 텍스트 실증) 5갈래 조사 결과 반영:
  ...
  - **계측기**: `metrics_v2.py` v2.4 로컬 확장 — `vague_noun_rate`(진단 신호) · `vague_noun_unsupported`(산문 전용, 게이트 아님) · `noun_string_max`·`light_verb_rate`(이 코퍼스에서 판별력 없음/죽은 지표, 관측만 — 별도 패턴 승격 안 함).
